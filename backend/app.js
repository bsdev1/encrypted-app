...
const { APPLICATION_CHUNK_SIZE } = require('../shared/constants');
...
io.on('connection', async (socket) => {
  ...
  socket.on('getChunks', async (uuid, cb) => {
    ...
    let offset = 0;
    const chunksLength = encryptedFile.length / APPLICATION_CHUNK_SIZE;
    ...
    for (let i = 0; i < chunks.length; i++) {
      ...
      const sequenceNumber = offset / APPLICATION_CHUNK_SIZE;

      const isFileLowerThanChunk =
        encryptedFile.length < APPLICATION_CHUNK_SIZE;
  ...
  socket.on(
    'newMessage',
    async ({ message, edited, files, fileDescriptions }, cb) => {
